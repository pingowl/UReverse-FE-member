                    ...
                    <div className={styles.receiptArea}>
                        <div className={styles.section}>
                            ...
                            <div className={styles.infoItem}>
                                ...
                                <div className={styles.infoItemContent}>{formData.address?.name}</div>
                            </div>
                            <div className={styles.infoItem}>
                                <div className={styles.infoItemTitle}>휴대폰번호</div>
                                <div className={styles.infoItemContent}>{formData.address?.phone}</div>
                            </div>
                            <div className={styles.infoItem}>
                                <div className={styles.infoItemTitle}>수거지 주소</div>
                                <div className={styles.infoItemContent}>
                                    ({formData.address?.zipCode}) {formData.address?.address} {formData.address?.addressDetail}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className={styles.section}>
                            ...
                            <div className={styles.brandInfo}>
                                <div className={styles.brandKo}>{formData.product?.brand?.name}</div>
                                <div className={styles.brandEn}>{formData.product?.brand?.nameEn}</div>
                            </div>
                            <div className={styles.infoItemContent}>
                                {formData.product.category?.mainCategoryName} | {formData.product.category?.subCategoryName}
                            </div>
                        ...
                        <div className={styles.section}>
                            ...
                            <div className={styles.subTitle}>아래 주의사항을 확인 후, 동의 바랍니다.</div>

                            <div className={styles.checkboxItem}>
                                <input className={styles.checkbox} type="checkbox" id="agree1" checked={agree1} onChange={() => setAgree1(!agree1)} />
                                <div>
                                    <label htmlFor="agree1">
                                        (필수) 개인정보 수집/이용 및 제3자 제공 약관 동의
                                    </label>
                                    <div className={styles.viewMore} onClick={() => window.open('https://mmemory.notion.site/3-1d531790a13680d493b3f32f6f66b539')}>
                                        자세히 보기
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className={styles.checkboxItem}>
                                <input className={styles.checkbox} type="checkbox" id="agree2" checked={agree2} onChange={() => setAgree2(!agree2)} />
                                <label htmlFor="agree2">
                                    (필수) 상품 수거 이후에는 판매 철회가 불가능하며, 검수 결과에 따라 실패된 상품만 반송될 수 있음에 동의합니다.
                                </label>
                            </div>
                        </div>
                    </div>
